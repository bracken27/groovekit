...
1. **Extract the ZIP**
2. **Initialize submodules** (requires Git installed):
   ```bash
   cd groovekit
   git init
   git submodule add https://github.com/Tracktion/tracktion_engine.git third-party/tracktion_engine
   cd third-party/tracktion_engine
   git submodule update --init --recursive
   cd ../..
   ```
...
9. **Export to audio** - Lastly, go to the "File" menu again and click "Export Audio" to immediately export your project
   to a .wav file. Now you can play your bop in your favorite .wave compatible audio player!
