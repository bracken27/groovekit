...
void TrackEditView::resized()
{
    ...
    // if (midiKeyboard)
    //     // [Generated by Junie] Reserve space at the bottom of the view for the on-screen MIDI keyboard.
    //     midiKeyboard->setBounds (r.removeFromBottom (keyboardHeight));
...
bool TrackEditView::processNoteKey (const char key)
{
    if (key == 'a' || key == 'A')
    {
    ...
    // else if (key == 'w' || key == 'W')
    // {
    //     midiKeyboardState.noteOn (1, keyboardBaseOctave + 1, 100.f);
    //     return true;
    // }
    return false;
}
